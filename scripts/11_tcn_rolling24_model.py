...
class Config:
    ...
    TRAINING_WINDOW_TYPE: str = "rolling"
    TRAINING_WINDOW_MONTHS: int = 24
    MODEL_NAME: str = "tcn_rolling24_selected"
    SMOKE_MODEL_NAME: str = "tcn_rolling24_smoke"
    CANDIDATE_MODEL_PREFIX: str = "tcn_rolling24__"
    ...
    RUNTIME_CALIBRATION_TASKS: Tuple[Tuple[str, str, str], ...] = (
        ("tcn_rolling24_168_c32_k3_do10_lr1em03_b64", "price", "NORD"),
        ("tcn_rolling24_168_c64_k3_do20_lr5em04_b64", "price", "CSUD"),
        ("tcn_rolling24_336_c32_k5_do20_lr1em03_b64", "purchases", "NORD"),
        ("tcn_rolling24_336_c64_k3_do20_lr5em04_b64", "purchases", "SICI"),
        ("tcn_rolling24_168_c32_k5_do20_lr1em03_b64", "sales", "CSUD"),
        ("tcn_rolling24_336_c64_k5_do20_lr5em04_b64", "sales", "SARD"),
    )
    ...
    FAST_STRATEGY_IDS: Tuple[str, ...] = (
        "tcn_rolling24_168_c32_k3_do10_lr1em03_b64",
        "tcn_rolling24_168_c64_k3_do20_lr5em04_b64",
        "tcn_rolling24_336_c32_k3_do10_lr1em03_b64",
        "tcn_rolling24_336_c64_k5_do20_lr5em04_b64",
    )
    ...
    SELECTED_STRATEGY_BY_TARGET: Dict[str, str] = field(
        default_factory=lambda: {
            "price": "tcn_rolling24_168_c32_k5_do10_lr1em03_b64",
            "purchases": "tcn_rolling24_168_c32_k5_do10_lr1em03_b64",
            "sales": "tcn_rolling24_168_c32_k3_do10_lr1em03_b64",
        }
    )
    ...
    FINAL_PRED_PARQUET_REL: str = "data/predictions/pred_tcn_rolling24.parquet"
    FINAL_PRED_RDS_REL: str = "data/predictions/pred_tcn_rolling24.rds"
    EXPERIMENT_DIR_REL: str = "experiments/tcn_rolling24_norecal"
...
def find_project_root() -> Path:
    ...
    The preferred launch pattern is `python scripts/11_tcn_rolling24_model.py` from the project
...
def make_strategy_grid(max_epochs: int, weight_norm: bool) -> List[TCNStrategy]:
    base = [
        ("tcn_rolling24_168_c32_k3_do10_lr1em03_b64", 168, [32, 32, 32, 32, 32, 32, 32], 3, 0.10, 1e-3, 64),
        ("tcn_rolling24_168_c32_k3_do20_lr1em03_b64", 168, [32, 32, 32, 32, 32, 32, 32], 3, 0.20, 1e-3, 64),
        ("tcn_rolling24_168_c64_k3_do10_lr5em04_b64", 168, [64, 64, 64, 64, 64, 64, 64], 3, 0.10, 5e-4, 64),
        ("tcn_rolling24_168_c64_k3_do20_lr5em04_b64", 168, [64, 64, 64, 64, 64, 64, 64], 3, 0.20, 5e-4, 64),
        ("tcn_rolling24_168_c32_k5_do10_lr1em03_b64", 168, [32, 32, 32, 32, 32, 32], 5, 0.10, 1e-3, 64),
        ("tcn_rolling24_168_c32_k5_do20_lr1em03_b64", 168, [32, 32, 32, 32, 32, 32], 5, 0.20, 1e-3, 64),
        ("tcn_rolling24_168_c64_k5_do10_lr5em04_b64", 168, [64, 64, 64, 64, 64, 64], 5, 0.10, 5e-4, 64),
        ("tcn_rolling24_168_c64_k5_do20_lr5em04_b64", 168, [64, 64, 64, 64, 64, 64], 5, 0.20, 5e-4, 64),
        ("tcn_rolling24_336_c32_k3_do10_lr1em03_b64", 336, [32, 32, 32, 32, 32, 32, 32, 32], 3, 0.10, 1e-3, 64),
        ("tcn_rolling24_336_c32_k3_do20_lr1em03_b64", 336, [32, 32, 32, 32, 32, 32, 32, 32], 3, 0.20, 1e-3, 64),
        ("tcn_rolling24_336_c64_k3_do10_lr5em04_b64", 336, [64, 64, 64, 64, 64, 64, 64, 64], 3, 0.10, 5e-4, 64),
        ("tcn_rolling24_336_c64_k3_do20_lr5em04_b64", 336, [64, 64, 64, 64, 64, 64, 64, 64], 3, 0.20, 5e-4, 64),
        ("tcn_rolling24_336_c32_k5_do10_lr1em03_b64", 336, [32, 32, 32, 32, 32, 32, 32], 5, 0.10, 1e-3, 64),
        ("tcn_rolling24_336_c32_k5_do20_lr1em03_b64", 336, [32, 32, 32, 32, 32, 32, 32], 5, 0.20, 1e-3, 64),
        ("tcn_rolling24_336_c64_k5_do10_lr5em04_b64", 336, [64, 64, 64, 64, 64, 64, 64], 5, 0.10, 5e-4, 64),
        ("tcn_rolling24_336_c64_k5_do20_lr5em04_b64", 336, [64, 64, 64, 64, 64, 64, 64], 5, 0.20, 5e-4, 64),
    ]
    strategies = [
        TCNStrategy(
            strategy_id=sid,
...
def setup_logging(paths: Dict[str, Path]) -> logging.Logger:
    logger = logging.getLogger("tcn_rolling24")
    ...
    file_handler = logging.FileHandler(paths["logs"] / "tcn_rolling24_run.log", mode="a", encoding="utf-8")
...
def select_training_dates(
    ...
    stage: str,
    cfg: Config,
    strategy: Optional[TCNStrategy] = None,
) -> Tuple[List[pd.Timestamp], List[pd.Timestamp]]:
    """Select leakage-safe training and inner-validation dates.

    This rolling-24 variant keeps RECALIBRATION_FREQUENCY = "none": for each
    target-zone-strategy task there is a single fit for the whole validation or
    test split. The "recalibration date" is therefore the last available date
    before the evaluated split:
        - validation/smoke/runtime: 31 Dec of VALIDATION_YEAR - 1;
        - final_test: 31 Dec of VALIDATION_YEAR.

    For rolling strategies, the outer training block starts 24 months before the
    recalibration date plus one day and ends at the recalibration date. The last
    INNER_VALIDATION_DAYS of this block are used for early stopping, as in the
    original direct multi-output TCN.
    """
    ...
    if stage in {"validation", "fast_validation", "runtime_calibration", "smoke"}:
        recalibration_date = validation_start - pd.Timedelta(days=1)
    elif stage == "final_test":
        recalibration_date = validation_end
    else:
        raise ValueError(f"Unknown training stage: {stage}")

    active_window_type = strategy.window_type if strategy is not None else cfg.TRAINING_WINDOW_TYPE
    active_window_months = strategy.window_months if strategy is not None else cfg.TRAINING_WINDOW_MONTHS

    if active_window_type == "rolling":
        if active_window_months is None:
            raise ValueError("Rolling TCN strategy requires window_months to be set.")
        window_start = (recalibration_date - pd.DateOffset(months=int(active_window_months))) + pd.Timedelta(days=1)
        outer_dates = [d for d in dates if window_start <= d <= recalibration_date]
    elif active_window_type == "expanding":
        outer_dates = [d for d in dates if d <= recalibration_date]
    else:
        raise ValueError(f"Unsupported TCN window_type: {active_window_type}")
...
def task_signature(
    ...
    payload = {
        ...
        "script_version": "tcn_rolling24_v1_direct_multioutput",
    }
...
def fit_predict_one_task(
    ...
    logger.info("Training rolling-24 TCN | stage=%s split=%s strategy=%s target=%s zone=%s", stage, split, active_strategy.strategy_id, target, zone)
    ...
    train_dates, inner_val_dates = select_training_dates(all_dates, stage, cfg, active_strategy)
    if train_dates and inner_val_dates:
        logger.info(
            "Rolling-24 window | stage=%s target=%s zone=%s strategy=%s train=%s..%s inner_val=%s..%s",
            stage,
            target,
            zone,
            ...
            pd.Timestamp(inner_val_dates[0]).date(),
            pd.Timestamp(inner_val_dates[-1]).date(),
        )
    ...
    log_payload = {
        ...
        "train_start_date": str(pd.Timestamp(train_dates[0]).date()) if train_dates else None,
        "train_end_date": str(pd.Timestamp(train_dates[-1]).date()) if train_dates else None,
        "inner_validation_start_date": str(pd.Timestamp(inner_val_dates[0]).date()) if inner_val_dates else None,
        "inner_validation_end_date": str(pd.Timestamp(inner_val_dates[-1]).date()) if inner_val_dates else None,
...
def load_selected_validation_predictions(selected: pd.DataFrame, paths: Dict[str, Path], cfg: Config, logger: logging.Logger) -> pd.DataFrame:
    ...
    if not files:
        logger.warning("No validation candidate prediction files found; final pred_tcn_rolling24 may contain test only.")
        return pd.DataFrame()
...
def main() -> None:
    ...
    logger.info("Starting rolling-24 TCN direct multi-output script. Project root: %s", cfg.PROJECT_ROOT)
    ...
    if cfg.RUN_SMOKE_TEST:
        ...
        smoke_pred["model"] = cfg.SMOKE_MODEL_NAME
        smoke_pred = validate_prediction_file(smoke_pred, eval_index, cfg, final_only=False)
        ...
        logger.info("Rolling-24 smoke test completed. Smoke predictions saved to pred_tcn_rolling24.parquet.")
        return
    ...
    if cfg.RUN_FULL_VALIDATION:
        logger.info("RUN_FULL_VALIDATION=True: running rolling-24 full 16-strategy validation grid.")
    ...
    if cfg.RUN_FINAL_TEST:
        logger.info("RUN_FINAL_TEST=True: training selected final rolling-24 TCN models and evaluating 2025 test period.")
        if selected.empty:
    ...
    logger.info("Rolling-24 TCN script completed.")
