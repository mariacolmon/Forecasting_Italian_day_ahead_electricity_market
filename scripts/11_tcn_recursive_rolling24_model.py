...
# -*- coding: utf-8 -*-
"""
11_tcn_recursive_rolling24_model.py
...
class Config:
    ...
    EVAL_INDEX_PARQUET_REL: str = "data/evaluation/eval_index_hourly.parquet"
    EVAL_INDEX_RDS_REL: str = "data/evaluation/eval_index_hourly.rds"
    WEEKLY_NAIVE_PARQUET_REL: str = "data/predictions/pred_naive_week_before.parquet"
    WEEKLY_NAIVE_RDS_REL: str = "data/predictions/pred_naive_week_before.rds"

    PREDICTIONS_DIR_REL: str = "data/predictions"
    FINAL_PREDICTION_PARQUET_REL: str = "data/predictions/pred_tcn_recursive_rolling24.parquet"
    FINAL_PREDICTION_RDS_REL: str = "data/predictions/pred_tcn_recursive_rolling24.rds"
    EXPERIMENT_DIR_REL: str = "experiments/tcn_recursive_rolling24"

    # Start safely. After smoke test, set RUN_SMOKE_TEST=False and RUN_FINAL_TEST=True.
    ...
    MODEL_NAME_FINAL: str = "tcn_recursive_rolling24_selected"
    MODEL_NAME_SMOKE: str = "tcn_recursive_rolling24_smoke"
    ...
    # Recursive one-step training has many more samples than direct daily multi-output.
    # The architecture parameters below keep the selected direct-TCN architecture,
    # while max_epochs is kept practical and early stopping remains active.
    MAX_EPOCHS: int = 20
    ...
    # Reuses the architecture selected in the original/direct TCN validation,
    # translated to recursive one-step strategy IDs.
    SELECTED_STRATEGY_BY_TARGET: Dict[str, str] = field(
        default_factory=lambda: {
            "price": "tcn_rec24_168_c32_k3_do10_lr1em03_b64",
            "purchases": "tcn_rec24_168_c64_k5_do10_lr5em04_b64",
            "sales": "tcn_rec24_168_c32_k5_do10_lr1em03_b64",
        }
    )
...
def setup_experiment_dirs(cfg: Config) -> Dict[str, Path]:
    root = root_path(cfg, cfg.EXPERIMENT_DIR_REL)
    paths = {
        "root": root,
        "logs": root / "logs",
    ...
    for path in paths.values():
        ensure_dir(path)
    ensure_dir(root_path(cfg, cfg.PREDICTIONS_DIR_REL))
    return paths


def setup_logging(paths: Dict[str, Path]) -> logging.Logger:
    logger = logging.getLogger("tcn_recursive_rolling24")
    ...
    file_handler = logging.FileHandler(paths["logs"] / "tcn_recursive_run.log", mode="a", encoding="utf-8")
...
def get_device(cfg: Config, logger: logging.Logger) -> torch.device:
...
def serialize_config(cfg: Config, strategies: Dict[str, TCNRecursiveStrategy]) -> Dict[str, Any]:
    out = asdict(cfg)
    out["PROJECT_ROOT"] = str(cfg.PROJECT_ROOT)
    out["strategies"] = [s.as_log_dict() for s in strategies.values()]
    return out
...
def load_table(parquet_path: Path, rds_path: Path, name: str, logger: logging.Logger) -> pd.DataFrame:
    if parquet_path.exists():
        logger.info("Reading %s from parquet: %s", name, parquet_path)
        return pd.read_parquet(parquet_path)
    if rds_path.exists():
        logger.info("Reading %s from RDS: %s", name, rds_path)
        return read_rds(rds_path)
    raise FileNotFoundError(f"Missing {name}. Expected {parquet_path} or {rds_path}.")


def load_inputs(cfg: Config, logger: logging.Logger) -> Tuple[pd.DataFrame, pd.DataFrame, Optional[pd.DataFrame]]:
    panel = load_table(
    ...
    eval_index = load_table(
        root_path(cfg, cfg.EVAL_INDEX_PARQUET_REL),
        root_path(cfg, cfg.EVAL_INDEX_RDS_REL),
        "common evaluation index",
        logger,
    )
    try:
        naive = load_table(
            root_path(cfg, cfg.WEEKLY_NAIVE_PARQUET_REL),
            root_path(cfg, cfg.WEEKLY_NAIVE_RDS_REL),
        ...
        naive = None
    return panel, eval_index, naive


def save_outputs(df: pd.DataFrame, parquet_path: Path, rds_path: Path, cfg: Config, logger: logging.Logger) -> None:
    atomic_write_parquet(df, parquet_path, engine=cfg.PARQUET_ENGINE)
    logger.info("Saved parquet: %s", parquet_path)
    if not cfg.TRY_WRITE_RDS_OUTPUT:
        return
    try:
        import pyreadr  # type: ignore
...
def normalize_datetime(values: Any) -> Any:
    if isinstance(values, pd.Series):
        return pd.to_datetime(values, utc=True, errors="coerce").dt.tz_convert(None)
    return pd.to_datetime(values, utc=True, errors="coerce").tz_convert(None)
...
def prepare_panel(panel: pd.DataFrame, cfg: Config, logger: logging.Logger) -> pd.DataFrame:
    ...
    if "holiday" in panel.columns:
        ...
        logger.info("Optional holiday column found and included as known covariate.")
    else:
...
def prepare_eval_index(eval_index: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.sort_values(["target", "zone", "split", "delivery_datetime_model"]).reset_index(drop=True)
...
def build_recursive_feature_frame(panel: pd.DataFrame, target: str, zone: str, cfg: Config) -> Tuple[pd.DataFrame, List[str]]:
    ...
    The only dynamic series updated recursively is the target being forecasted.
    Same-day realised market variables are not used.
    """
...
class TCNEncoder(nn.Module):
    def __init__(self, input_size: int, num_channels: Sequence[int], kernel_size: int, dropout: float, weight_norm: bool):
        ...
        for i, out_ch in enumerate(num_channels):
            layers.append(TemporalBlock(in_ch, out_ch, kernel_size, 2 ** i, dropout, weight_norm))
            in_ch = out_ch
        self.network = nn.Sequential(*layers)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Input: (batch, sequence_length, features). Conv1d: (batch, channels, sequence_length).
        return self.network(x.permute(0, 2, 1))


class RecursiveTCN(nn.Module):
    def __init__(self, input_size: int, cov_size: int, strategy: TCNRecursiveStrategy, covariate_hidden_dim: int) -> None:
        super().__init__()
        ...
        self.covariate_branch = nn.Sequential(
            nn.Linear(cov_size, covariate_hidden_dim),
            nn.ReLU(),
            nn.Dropout(strategy.dropout),
        )
        self.head = nn.Sequential(
            nn.Linear(last_channels + covariate_hidden_dim, 64),
            nn.ReLU(),
            nn.Dropout(strategy.dropout),
            nn.Linear(64, 1),
        )
    ...
    def forward(self, x_seq: torch.Tensor, x_cov: torch.Tensor) -> torch.Tensor:
        ...
        cov_repr = self.covariate_branch(x_cov)
        return self.head(torch.cat([last_hidden, cov_repr], dim=1))
...
def train_model(
    ...
    x_val_cov: np.ndarray,
    y_val: np.ndarray,
    strategy: TCNRecursiveStrategy,
    cfg: Config,
    device: torch.device,
    ...
    train_loader = DataLoader(
        HourlyDataset(x_train_seq, x_train_cov, y_train),
    ...
    val_loader = DataLoader(
        HourlyDataset(x_val_seq, x_val_cov, y_val),
        batch_size=strategy.batch_size,
        shuffle=False,
        num_workers=cfg.NUM_WORKERS,
        drop_last=False,
    ...
    model = RecursiveTCN(
        ...
        strategy=strategy,
        covariate_hidden_dim=cfg.COVARIATE_HIDDEN_DIM,
    ).to(device)
...
def predict_scaled_one(model: RecursiveTCN, x_seq: np.ndarray, x_cov: np.ndarray, device: torch.device) -> float:
    model.eval()
    with torch.no_grad():
...
def make_default_strategies(cfg: Config) -> Dict[str, TCNRecursiveStrategy]:
    specs = [
        # Baseline close to the selected direct TCN for price/purchases.
        (
            "tcn_rec24_168_c32_k5_do10_lr1em03_b64",
            168,
            [32, 32, 32, 32, 32, 32],
            5,
            0.10,
        ...
        # More local convolution, useful especially because sales had selected k=3 before.
        (
            "tcn_rec24_168_c32_k3_do10_lr1em03_b64",
            168,
        ...
        # Longer look-back window: tests whether the recursive model benefits from more history.
        (
            "tcn_rec24_336_c32_k5_do10_lr1em03_b64",
            336,
            [32, 32, 32, 32, 32, 32, 32],
            5,
            0.10,
        ...
        # Higher-capacity version with lower learning rate, close to the original TCN grid.
        (
            "tcn_rec24_168_c64_k5_do10_lr5em04_b64",
            168,
            [64, 64, 64, 64, 64, 64],
            5,
            0.10,
    ...
    strategies = [
        ...
        for sid, lookback, channels, kernel, dropout, lr, batch in specs
    ]

    for strategy in strategies:
        if strategy.receptive_field < 168:
            raise ValueError(
                f"Strategy {strategy.strategy_id} has receptive field {strategy.receptive_field}, below 168."
            )

    return {s.strategy_id: s for s in strategies}

def make_smoke_strategy(cfg: Config) -> TCNRecursiveStrategy:
    return TCNRecursiveStrategy(
        strategy_id="tcn_rec24_smoke_168h",
        lookback_hours=168,
        window_months=cfg.WINDOW_MONTHS,
        num_channels=[16, 16, 16, 16, 16, 16, 16],
        kernel_size=3,
        dropout=0.10,
        learning_rate=1e-3,
        batch_size=128,
        max_epochs=cfg.SMOKE_MAX_EPOCHS,
        weight_norm=cfg.WEIGHT_NORM,
    )


def fast_strategy(strategy: TCNRecursiveStrategy, cfg: Config) -> TCNRecursiveStrategy:
    s = TCNRecursiveStrategy(**asdict(strategy))
    s.max_epochs = cfg.FAST_MAX_EPOCHS
    return s
...
def mode_strategies(mode: str, cfg: Config, strategy_dict: Dict[str, TCNRecursiveStrategy], selected: Dict[str, str]) -> List[TCNRecursiveStrategy]:
    if mode == "smoke":
...
def limit_eval_dates(eval_tz: pd.DataFrame, mode: str, cfg: Config) -> pd.DataFrame:
    ...
    if mode == "smoke":
        dates = out["delivery_date"].drop_duplicates().sort_values().head(cfg.SMOKE_MAX_FORECAST_DATES)
        out = out[out["delivery_date"].isin(dates)].copy()
    elif mode == "fast_validation" and cfg.FAST_MAX_FORECAST_DATES_PER_SERIES is not None:
        dates = out["delivery_date"].drop_duplicates().sort_values().tail(cfg.FAST_MAX_FORECAST_DATES_PER_SERIES)
        out = out[out["delivery_date"].isin(dates)].copy()
    return out


def fixed_recalibration_date(mode: str, cfg: Config) -> pd.Timestamp:
    if mode == "final_test":
        return pd.Timestamp(f"{cfg.VALIDATION_YEAR}-12-31")
    return pd.Timestamp(f"{cfg.VALIDATION_YEAR - 1}-12-31")


def recalibration_blocks(eval_tz: pd.DataFrame, mode: str, cfg: Config) -> List[Tuple[pd.Timestamp, pd.DataFrame]]:
    if eval_tz.empty:
        return []
    ...
    if freq in {"none", "no", "fixed", "no_recalibration"}:
        return [(fixed_recalibration_date(mode, cfg), temp)]
    if freq == "monthly":
        temp["forecast_month"] = temp["forecast_date"].dt.to_period("M")
        return [(pd.Timestamp(block["forecast_date"].min()).normalize(), block.drop(columns=["forecast_month"])) for _, block in temp.groupby("forecast_month", sort=True)]
    raise ValueError(f"Unsupported RECALIBRATION_FREQUENCY: {cfg.RECALIBRATION_FREQUENCY}")


def train_window(recalibration_date: pd.Timestamp, strategy: TCNRecursiveStrategy) -> Tuple[pd.Timestamp, pd.Timestamp]:
...
def checkpoint_path(cfg: Config, split: str, sid: str, target: str, zone: str, recal: pd.Timestamp) -> Path:
    name = "__".join([
        ...
        safe_filename(zone),
        pd.Timestamp(recal).strftime("%Y-%m-%d"),
    ]) + ".parquet"
    return root_path(cfg, cfg.EXPERIMENT_DIR_REL) / "checkpoints" / name
...
def curve_path(cfg: Config, split: str, sid: str, target: str, zone: str, recal: pd.Timestamp) -> Path:
    ...
    return root_path(cfg, cfg.EXPERIMENT_DIR_REL) / "training_curves" / name
...
def recursive_predict_block(
    ...
    for _, drow in days.iterrows():
        ...
        # Restore realised values for later forecast origins. The recursion is only within the day.
    ...
    ordered = [
        ...
        "strategy_id", "lookback_hours", "window_type", "window_months", "kernel_size", "num_channels", "dilations", "receptive_field",
        "train_start_date", "train_end_date", "recalibration_date", "dropout", "learning_rate", "batch_size", "weight_norm", "weight_decay",
        "epochs_trained", "best_validation_loss", "fit_seconds", "predict_seconds"
    ]
...
def run_one_block(
    ...
    try:
        ...
        logger.info(
            "Recursive TCN rolling-24 window | split=%s target=%s zone=%s strategy=%s train_window=%s..%s",
            split, target, zone, strategy.strategy_id, tr_start.date(), tr_end.date(),
        )
        ...
        # Fit preprocessors only on training information, not on inner validation or test.
        seq_fit_positions = np.unique(np.concatenate([positions[i] for i in train_idx]))
        seq_prep = SequencePreprocessor(list(feat.columns)).fit(feat.iloc[seq_fit_positions])
        ...
        result = train_model(x_tr_seq, x_tr_cov, y_tr, x_va_seq, x_va_cov, y_va, strategy, cfg, device, logger)
        ...
        pred, predict_seconds = recursive_predict_block(
            result.model,
            feat,
            eval_block,
            seq_prep,
        ...
        if cfg.SAVE_FITTED_MODELS:
            model_path = root_path(cfg, cfg.EXPERIMENT_DIR_REL) / "model_artifacts" / f"{safe_filename(split)}__{safe_filename(strategy.strategy_id)}__{safe_filename(target)}__{safe_filename(zone)}.pt"
            ensure_dir(model_path.parent)
            torch.save({"model_state_dict": result.model.state_dict(), "strategy": strategy.as_log_dict()}, model_path)
        ...
        log.update({
            ...
            "n_sequence_features": hist_matrix.shape[1],
            "n_covariates": len(cov_cols),
            "inner_validation_start_date": pd.Timestamp(ts[val_idx[0]]).date() if len(val_idx) else None,
            "inner_validation_end_date": pd.Timestamp(ts[val_idx[-1]]).date() if len(val_idx) else None,
        })
...
def run_strategy_target_zone(
    ...
    logger.info("Running recursive TCN | mode=%s split=%s strategy=%s target=%s zone=%s", mode, split, strategy.strategy_id, target, zone)
    ...
    pred_parts, logs = [], []
    for recal, block in recalibration_blocks(eval_tz, mode, cfg):
    ...
    if split == "validation" and not pred_df.empty and mode != "smoke":
        out_dir = root_path(cfg, cfg.EXPERIMENT_DIR_REL) / "validation_candidate_predictions"
        ensure_dir(out_dir)
        pred_df.to_parquet(
            out_dir / f"pred_{safe_filename(strategy.strategy_id)}__{safe_filename(target)}__{safe_filename(zone)}.parquet",
            index=False,
        )
...
def prepare_weekly_naive(naive: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
    if naive is None or naive.empty:
        return None
    out = naive.copy()
    required = ["target", "zone", "split", "delivery_datetime_model", "y_pred"]
    if any(c not in out.columns for c in required):
        return None
    out["delivery_datetime_model"] = normalize_datetime(out["delivery_datetime_model"])
    if "model" in out.columns:
        out = out[out["model"].astype(str) == "naive_week_before"].copy()
    out = out.rename(columns={"y_pred": "y_pred_naive_week"})
    return out[["target", "zone", "split", "delivery_datetime_model", "y_pred_naive_week"]]
...
def compute_quick_metrics(pred: pd.DataFrame, naive: Optional[pd.DataFrame]) -> pd.DataFrame:
    if pred.empty:
        return pd.DataFrame()
    df = pred.copy()
    df["delivery_datetime_model"] = normalize_datetime(df["delivery_datetime_model"])
    ...
    naive_df = prepare_weekly_naive(naive)
    ...
    joined["naive_abs_error"] = (joined["y_true"] - joined["y_pred_naive_week"]).abs()
...
def select_best_by_target(metrics: pd.DataFrame, cfg: Config, paths: Dict[str, Path], logger: logging.Logger) -> Dict[str, str]:
    ...
    atomic_write_csv(scores, paths["selected_strategy"] / "tcn_recursive_validation_strategy_results.csv")
    atomic_write_csv(
        pd.DataFrame([{"target": k, "strategy_id": v, "selection_metric": score_col} for k, v in selected.items()]),
        paths["selected_strategy"] / "tcn_recursive_selected_strategy_by_target.csv",
    )
    logger.info("Selected recursive TCN strategies by target using %s: %s", score_col, selected)
    return selected


def save_logs_metrics(logs: pd.DataFrame, pred: pd.DataFrame, naive: Optional[pd.DataFrame], paths: Dict[str, Path], logger: logging.Logger) -> pd.DataFrame:
    if not logs.empty:
        atomic_write_csv(logs, paths["logs"] / "tcn_recursive_training_logs.csv")
        logger.info("Saved training logs: %s", paths["logs"] / "tcn_recursive_training_logs.csv")
    metrics = compute_quick_metrics(pred, naive)
    if not metrics.empty:
        atomic_write_csv(metrics, paths["logs"] / "tcn_recursive_quick_metrics.csv")
        logger.info("Saved quick metrics: %s", paths["logs"] / "tcn_recursive_quick_metrics.csv")
    return metrics
...
def validate_prediction_output(pred: pd.DataFrame, cfg: Config) -> None:
    ...
    dup = pred.duplicated(["model", "target", "zone", "split", "delivery_datetime_model"]).sum()
    if dup > 0:
        raise ValueError(f"Prediction output contains {dup} duplicated model-target-zone-split-datetime rows.")
    bad_h = sorted(set(pred["horizon"].dropna().astype(int)) - set(range(1, 25)))
    if bad_h:
        raise ValueError(f"Unexpected horizons: {bad_h}")
    counts = pred.groupby(["model", "target", "zone", "split", "delivery_date"])["hour"].nunique().reset_index(name="n_hours")
...
def print_quality_summary(pred: pd.DataFrame, logger: logging.Logger) -> None:
    logger.info("================ RECURSIVE TCN PREDICTION QUALITY SUMMARY ================")
...
def run_mode(
    ...
    eval_index: pd.DataFrame,
    cfg: Config,
    strategy_dict: Dict[str, TCNRecursiveStrategy],
        ...
        for target in targets:
            for zone in zones:
                if mode == "final_test" and strategy.strategy_id != selected.get(target, cfg.SELECTED_STRATEGY_BY_TARGET.get(target)):
                    continue
                model_name = cfg.MODEL_NAME_SMOKE if mode == "smoke" else (cfg.MODEL_NAME_FINAL if mode == "final_test" else f"tcn_recursive__{strategy.strategy_id}")
...
def main() -> None:
    ...
    logger.info("Starting 12_tcn_recursive_rolling24_model.py")
    logger.info("Project root: %s", cfg.PROJECT_ROOT)
    logger.info("Device: %s", device)
    logger.info("Enabled modes: %s", resolve_modes(cfg))

    with open(paths["logs"] / "tcn_recursive_config.json", "w", encoding="utf-8") as f:
        json.dump(serialize_config(cfg, strategy_dict), f, indent=2, default=str)

    panel_raw, eval_raw, naive = load_inputs(cfg, logger)
    panel = prepare_panel(panel_raw, cfg, logger)
    eval_index = prepare_eval_index(eval_raw)
    ...
    for mode in resolve_modes(cfg):
        ...
        if mode in {"fast_validation", "full_validation"} and not pred_mode.empty:
            ...
            current_metrics = save_logs_metrics(current_logs, current_preds, naive, paths, logger)
            selected = select_best_by_target(current_metrics, cfg, paths, logger)
    ...
    if final_pred.empty:
        ...
        if not final_logs.empty:
            save_logs_metrics(final_logs, final_pred, naive, paths, logger)
        return
    ...
    save_outputs(
        final_pred,
        root_path(cfg, cfg.FINAL_PREDICTION_PARQUET_REL),
        root_path(cfg, cfg.FINAL_PREDICTION_RDS_REL),
        cfg,
        logger,
    )

    logger.info("Finished successfully.")
    logger.info("Main prediction output: %s", root_path(cfg, cfg.FINAL_PREDICTION_PARQUET_REL))
