...
class Config:
    ...
    FINAL_PREDICTION_PARQUET: Path = _DEFAULT_PROJECT_ROOT / "data/predictions/pred_lstm.parquet"
    FINAL_PREDICTION_RDS: Path = _DEFAULT_PROJECT_ROOT / "data/predictions/pred_lstm.rds"

    EXPERIMENT_DIR: Path = _DEFAULT_PROJECT_ROOT / "experiments/lstm_norecal"
    ...
    SELECTED_STRATEGY_BY_TARGET = {
        "price": "lstm_expanding_168h_h64_l2_lr5em04_b32",
        "purchases": "lstm_expanding_336h_h64_l1_lr5em04_b32",
        "sales": "lstm_expanding_336h_h64_l2_lr5em04_b32",
    }
...
def make_default_strategies(cfg: Config) -> Dict[str, LSTMStrategy]:
    """Create the LSTM validation grid.

    The LSTM uses an expanding training window by default, so that each
    recalibration exploits all past information available at that point.
    The validation grid focuses on architecture and optimisation choices:
    look-back length, hidden-state dimension, number of recurrent layers,
    learning rate and batch size. Dropout is determined by the number of
    LSTM layers to control computational cost.
    """
    strategies: List[LSTMStrategy] = []

    for lookback in cfg.LOOKBACK_OPTIONS:
        for hidden in cfg.HIDDEN_SIZE_OPTIONS:
            for layers in cfg.NUM_LAYERS_OPTIONS:
                for lr in cfg.LEARNING_RATE_OPTIONS:
                    for batch in cfg.BATCH_SIZE_OPTIONS:
                        dropout = cfg.DROPOUT_ONE_LAYER if layers == 1 else cfg.DROPOUT_TWO_LAYERS

                        lr_label = f"{lr:.0e}".replace("-", "m")
                        strategy_id = (
                            f"lstm_expanding_{lookback}h"
                            f"_h{hidden}"
                            f"_l{layers}"
                            f"_lr{lr_label}"
                            f"_b{batch}"
...
def get_mode_strategies(
    ...
    if mode == "fast_validation":
        selected_ids = [
            "lstm_expanding_168h_h32_l1_lr1em03_b64",
            "lstm_expanding_336h_h64_l2_lr1em03_b64",
        ]
...
def main() -> None:
    ...
    logger.info("Starting 09_lstm_model.py")
